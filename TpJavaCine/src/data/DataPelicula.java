...
public class DataPelicula extends Conexion{
	...
	public ArrayList<Pelicula> GetAll(){ 
		...
		{			
					...
					a.setDescPelicula(rs.getString("descPelicula"));
	...
	public boolean AgregarPeli(Pelicula peli) {
		...
		PreparedStatement pst = null;
		try {
			String consulta = "insert into peliculas (codPelicula, descPelicula, nombrePelicula)values (?,?,?)";
	...
	public Pelicula GetOne(int id) {
		...
		String consulta = "select * from pelicula where codPelicula=?";		
		PreparedStatement pst=null;
		ResultSet rs=null;
		try {
			pst = getConn().prepareStatement(consulta);
...
public void Update(Pelicula peli) {
		...
		ResultSet rs=null;
		try {
			String consulta = "Update peliculas set descPelicula=?,nombrePelicula=? where codPelicula==?";
			pst = getConn().prepareStatement(consulta);
